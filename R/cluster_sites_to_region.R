...
cluster_sites_to_region <- function(methylation,
  ...
  if(is_parallel){
    ...
    res <- foreach::`%dopar%`(obj = foreach::foreach(i=1:length(annotation),.packages = "BSDMR",.inorder = TRUE,
                              ...
                              ex = {out <- .parallel_find_region(methylation = methylation,annotation = annotation,index = i,min_sites_number = min_sites_number,max_distance_between_sites = max_distance_between_sites)})
